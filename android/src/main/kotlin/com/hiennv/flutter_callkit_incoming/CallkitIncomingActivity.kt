...
class CallkitIncomingActivity : Activity() {
    ...
    private fun onAcceptClick() {
        ...
        ivAcceptCall.isEnabled = false
        ivDeclineCall.isEnabled = false
        ivAcceptCall.alpha = 0.5f
        ivDeclineCall.alpha = 0.5f
        
        val data = intent.extras?.getBundle(EXTRA_CALLKIT_INCOMING_DATA)
        
        // Safely get the extra data and save order ID
        try {
            val extraData = data?.getSerializable("EXTRA_CALLKIT_EXTRA")
            if (extraData is HashMap<*, *>) {
                val map = extraData as? HashMap<String, Any?>
                // Try to get case_id first, then fall back to id
                val orderId = map?.get("case_id")?.toString() 
                    ?: map?.get("id")?.toString() 
                    ?: ""
                
                if (orderId.isNotEmpty()) {
                    val prefs: SharedPreferences = getSharedPreferences("FlutterSharedPreferences", Context.MODE_PRIVATE)
                    prefs.edit().putString("flutter.order_id", orderId).apply()
                }
            }
        } catch (e: Exception) {
            // Ignore errors when saving order ID
            e.printStackTrace()
        }

        // Stop the sound player service
        try {
            stopService(Intent(this@CallkitIncomingActivity, CallkitSoundPlayerService::class.java))
        } catch (e: Exception) {
            e.printStackTrace()
        }
        ...
        try {
            val callkitNotificationManager = CallkitNotificationManager(this@CallkitIncomingActivity)
            if (data != null) {
        ...
        // Send accept broadcast to notify Flutter
