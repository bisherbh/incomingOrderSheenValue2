...
/// Instance to use library functions for incoming order notifications.
/// * showCallkitIncoming(dynamic) - Show incoming order notification
/// * startCall(dynamic) - Start viewing an order
/// * endCall(dynamic) - End order session
/// * endAllCalls() - End all active order sessions
...
class FlutterCallkitIncoming {
  ...
  /// Event.ACTION_CALL_INCOMING - Received an incoming order
  /// Event.ACTION_CALL_START - Started viewing an order
  /// Event.ACTION_CALL_ACCEPT - Accepted/viewed an incoming order
  /// Event.ACTION_CALL_DECLINE - Declined/ignored an incoming order
  /// Event.ACTION_CALL_ENDED - Ended an order session
  /// Event.ACTION_CALL_TIMEOUT - Missed an incoming order
  /// Event.ACTION_CALL_CALLBACK - only Android (click action `View Order` from missed order notification)
  ...
  /// Show missed order notification.
  /// Only Android
  ...
  /// End all order sessions.
  static Future endAllCalls() async {
    await _channel.invokeMethod("endAllCalls");
  }

  /// Get active order sessions.
  /// On iOS: return active sessions from Callkit.
  /// On Android: only return last session
