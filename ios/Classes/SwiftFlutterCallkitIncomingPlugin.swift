...
public class SwiftFlutterCallkitIncomingPlugin: NSObject, FlutterPlugin, CXProviderDelegate {
    ...
    public func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
        ...
        // Safely save order ID - try case_id first, then id
        if let extraData = self.data?.extra {
            let orderId = extraData["case_id"] ?? extraData["id"] ?? ""
            if let orderIdString = orderId as? String, !orderIdString.isEmpty {
                UserDefaults.standard.set(orderIdString, forKey: "flutter.order_id")
            }
        }
